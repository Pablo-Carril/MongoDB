...
import { Camaramodel } from '../models/camara.model.js'
import { DateTime } from 'luxon'
import formateaFecha from '../utils.js'
const camarasRouter = Router()
...
camarasRouter.get('/:serie', async (req, res) => {     //api/equipos/serie
...
camarasRouter.post('/', async (req, res) => {       //DE ALGUNA FORMA TENGO QUE PASAR EQUIPO A "TODOS" luego de GUARDAR
...
camarasRouter.get('/edit/:id', async (req, res) => {
...
//ACTUALIZAR
camarasRouter.put('/actualizar/:id', async (req, res) => {
  const id = req.params.id
...
//BORRAR
camarasRouter.delete('/delete/:id', async (req, res) => {   //'/equipos/:id'
  //res.render('popup')
  try {
    const id = req.params.id
    const { body } = req
...
export default camarasRouter       //para importar en app.js
